let rmaBuilder = new Reef('#rma-builder', {
    ...
    template: function (props) {
        return `
            ...
            <textarea data-rma-action="change-builder-script" id="builder-script" rows="10"></textarea>

            <div><button data-rma-action="builder-compile-script" id="builder-compile-script">Compile</button></div>

            <div id="actions" style="height: 10rem; overflow-y: auto;">
                ${props.actions.map(action => `<div class="action ${action.isRunning ? 'is-running' : ''}">${action.getDescription()}</div>`).join('')}
            </div>

            <div class="flex justifySpaceBetween alignItemsCenter gap-10" style="margin: .5rem 0; ">
                ...
                <button data-rma-action="builder-reset" id="builder-reset">Reset</button>
...
const executeScript = async () => {
    ...
    // console.log(props.state, STATE_BUILDER_STOPPED);
    // if (props.state === STATE_BUILDER_STOPPED) return;
        ...
        for (const action of ALL_ACTIONS) {
            ...
            if (matches) {
                ...
                switch (action.name) {
                    ...
                    case "WaitXTime":
                        actions.push(new WaitXTime(parseInt(matches.groups.option, 10)))
                        break;
                    default:
                        break;
                }
